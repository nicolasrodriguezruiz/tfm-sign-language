...
class S2T_Dataset(Dataset.Dataset):
    ...
    def __init__(self, path, tokenizer, config, args, phase, training_refurbish=False):
        ...
        self.prompt_str = config["data"].get('prompt', {})
    ...
    def augment_preprocess_inputs(self, is_train, keypoints=None):
        ...
        if is_train == 'train':
            ...
            # Joint dropout estructurado
            if self.augmentation_cfg.get('structured_dropout', 0) > 0:
                for i in range(keypoints.shape[0]):
                    keypoints[i] = self.random_joint_dropout_2(
                        keypoints[i],
                        dropout_prob=self.augmentation_cfg['structured_dropout'],
                    )
            # Ruido gaussiano
    ...
    def _tokenize_text_for_qwen(self, text_batch):
        ...
        """
        # El Prompt (Sin añadir el EOS como texto)
