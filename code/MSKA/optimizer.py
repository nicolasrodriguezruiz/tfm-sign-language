...
def build_optimizer(config: dict, model) -> Optimizer:
    """
    Construye el optimizador con soporte para learning rates distintos por módulo.

    El config define un learning rate 'default' y opcionalmente learning rates
    específicos para módulos concretos del modelo. Esto es útil para fine-tuning:
    por ejemplo, usar un lr bajo para el backbone preentrenado y un lr más alto
    para las capas nuevas.

    Ejemplo de config:
        learning_rate:
            default: 1e-3
            visual_backbone: 1e-4    # backbone va más lento
            translation_network: 5e-4

    Optimizadores disponibles: adam, adamw, adagrad, adadelta, rmsprop, sgd.
    """
    ...
    # Extraer el lr por defecto del config (pop lo elimina del dict para no interferir)
    base_lr = config['learning_rate'].pop('default')

    # Asignar un lr a cada módulo hijo del modelo.
    # Si el nombre del módulo coincide con alguna clave del config, usa ese lr específico.
    # Si no coincide con ninguna, usa base_lr.
    parameters = []
    for module_name, module in model.named_children():
        lr_ = base_lr
        for config_key, lr in config['learning_rate'].items():
            if config_key in module_name:
                lr_ = lr
        parameters.append({'params': module.parameters(), 'lr': lr_})

    if optimizer_name == "adam":
        # Adam: adaptativo por parámetro. El más usado en deep learning.
        # Combina momentum (media móvil de gradientes) con RMSProp (media móvil de gradientes²).
        # betas=(0.9, 0.999): factor de decaimiento para la media de gradientes y su cuadrado.
        # amsgrad: variante que garantiza convergencia en algunos casos teóricos.
