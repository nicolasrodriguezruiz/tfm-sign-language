...
def build_prompt(examples, test_gloss):
    """
    Construye un prompt few-shot con K ejemplos glosa→traducción
    más la muestra a traducir.

    Args:
        examples:   lista de tuplas (glosa, traducción) del train set.
        test_gloss: string de glosas a traducir.

    Returns:
        String con el prompt completo listo para Qwen.
    """
    prompt = (
        "Traducir glosas de lengua de señas alemana al alemán natural.\n"
        "Las glosas son palabras clave en mayúsculas. "
        "La traducción debe ser una frase natural en alemán.\n\n"
    )
    for gloss, text in examples:
        prompt += f"Glosas: {gloss}\nTraducción: {text}\n\n"
    prompt += f"Glosas: {test_gloss}\nTraducción:"
    return prompt
    ...
    def evaluate_split(loader, split_name):
        ...
        with torch.no_grad():
            for step, src_input in enumerate(loader):
                ...
                for name, pred_gloss_ids, ref_gloss, ref_text in zip(
                    src_input['name'],
                    pred_gloss_batch,
                    src_input['gloss'],
                    src_input['text'],
                ):
                    # Construir string de glosas predichas
                    pred_gloss_str = ' '.join(pred_gloss_ids).upper()

                    # Construir prompt few-shot
                    prompt = build_prompt(examples, pred_gloss_str)
                    ...
                    output_ids = qwen_model.generate(
                        **inputs,
                        ...
                        eos_token_id=[
                            ...
                            newline_token_id,  # parar también en salto de línea
                        ],
                    ...
                    results[name]['prompt']   = prompt  # para inspección manual
