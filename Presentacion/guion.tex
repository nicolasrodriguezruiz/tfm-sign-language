...
El trabajo ha estudiado la viabilidad de sustituir el decodificador clásico mBART, dentro de un sistema basado en \textit{keypoints}, por un modelo pequeño de lenguaje de arquitectura \textit{decoder-only}, integrando Qwen2.5 en el reconocedor MSKA-SLR sobre el corpus PHOENIX-2014T. La conclusión general es que el enfoque es viable, pero no logra superar la línea base en el rango de modelos explorado.

La hipótesis principal, por tanto, no se confirma. Ninguna configuración de Qwen supera a mBART, que se mantiene en veintidós con cuarenta y ocho de BLEU-4. La mejor variante propia, el modelo de mil quinientos millones de parámetros con adaptadores completos y sin \textit{prompt}, se queda en veinte con cincuenta y cinco, a menos de dos puntos de distancia, y el modelo de siete mil millones no reduce esa brecha. La ventaja de mBART se explica por su arquitectura \textit{encoder-decoder} con atención cruzada, concebida para traducción, y por un preentrenamiento que incluye alemán de forma masiva, factores que compensan con eficacia los errores del reconocedor visual.

De este resultado se derivan cuatro claves. La primera es que el verdadero cuello de botella son los datos: el reconocedor se estanca en torno a un veintisiete por ciento de WER, el aumento de datos apenas resta una décima y las curvas evidencian un sobreajuste que la regularización no corrige, todo ello con poco más de siete mil muestras. La segunda es que el tamaño del modelo base pesa más que la estrategia de ajuste, como demuestra que el modelo de siete mil millones con LoRA en la atención supere al \textit{finetuning} completo del modelo pequeño actualizando una fracción de los parámetros. La tercera es que el \textit{prompt} de instrucción resulta contraproducente bajo \textit{finetuning}, ya que compite con los tokens visuales por la atención; eliminarlo recupera casi diez puntos. Y la cuarta es que el enfoque \textit{zero-shot} es insuficiente para una traducción precisa: los fallos de alucinación multilingüe y de ruptura de formato son de naturaleza estructural, pues aparecen incluso con glosas de referencia perfectas.

La tendencia de escalado resulta prometedora, aunque no concluyente. La mejora de casi cuatro puntos al pasar del modelo pequeño al de siete mil millones sugiere que la brecha con mBART podría cerrarse con modelos mayores, pero la evaluación del modelo de catorce mil millones con \textit{finetuning} excede la memoria disponible en una única GPU, de modo que esa hipótesis no puede verificarse dentro del alcance de este trabajo.

A partir de aquí se abren varias líneas de continuación. La más directa es el \textit{finetuning} de modelos de mayor tamaño, comprobando si la eliminación del \textit{prompt} sigue siendo beneficiosa a mayor escala. En paralelo, cabe explorar modelos de lenguaje especializados en alemán, como LL\"aMmlein, que podrían ofrecer una ventaja competitiva en este corpus, así como modelos de mayor escala en el escenario \textit{zero-shot}, donde las capacidades de seguimiento de instrucciones emergen con el tamaño. En el plano arquitectónico, integrar modelos con visión nativa que operen directamente sobre los píxeles permitiría preservar información gestual que hoy se pierde en la discretización al esqueleto, y el aprendizaje \textit{few-shot} ofrece una vía intermedia entre el \textit{zero-shot} y el \textit{finetuning}. Por último, en cuanto a la generalización, conviene validar el sistema sobre otros corpus, como CSL-Daily o How2Sign, y trasladar la metodología a otras lenguas de signos, en particular la Lengua de Signos Española, aprovechando modelos de lenguaje especializados en español.

Con esto concluyo la exposición. Muchas gracias por su atención.
