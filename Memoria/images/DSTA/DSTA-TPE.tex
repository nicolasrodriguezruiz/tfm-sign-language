...
%     inner frame sep=0pt, % <--- ¡ESTA LÍNEA ELIMINA EL MARGEN EXTRA!
    ...
    \node[color_tinta] (titulo) {\underline{\textbf{PE temporal}}};
    ...
    \node[mathnode, right=-0.25cm of x1_add, color_tinta] (vec1) {$\begin{bmatrix} 1 \\ 1 \\ 1 \end{bmatrix}$};
    ...
    \node[mathnode, right=-0.25cm of x2_add, color_tinta] (vec2) {$\begin{bmatrix} 2 \\ 2 \\ 2 \end{bmatrix}$};

    % 4. Nota explicativa lateral
    \node[texto, right=2.5cm of vec1, yshift=-1.1cm, color_tinta] (nota) {Mismo fotograma \\ misma\\ codificación posicional.\\};
    ...
    \draw[->, color_tinta, thick, rounded corners=3pt] ($(joint.east) + (0.5,0.25)$) -- ++(-0.5,0) |- (vec1.east);
    \draw[->, color_tinta, thick, rounded corners=3pt] ($(joint.east) + (0.5,0.25)$) -- ++(-0.5,0) |- (vec2.east);
